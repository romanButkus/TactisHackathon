...
def detect_objects(img, sector_id, origin, size, global_map_dim, obj_start_id=0):
    ...
    for profile in OBJECT_PROFILES:
        ...
        for cnt in contours:
            ...
            # x, y are the top-left pixel coordinates relative to the cropped image
            ...
            detections.append(
                {
                    ...
                    "type": profile["type"],
                    "sector": sector_id,
                    # Storing local picture center coordinates instead of global map coordinates
                    "local_pixel_center": [local_cx, local_cy],
                    # Bounding box coordinates: [x_min, y_min, width, height] of the object within the sector image
                    "local_bbox": [x, y, w, h],
                }
            )
    ...
    for r in range(grid_size):
        for c in range(grid_size):
            ...
            for d in detections:
                ...
                # FIX: Read the updated key 'local_bbox' instead of 'bbox'
